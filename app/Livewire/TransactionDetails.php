...
class TransactionDetails extends Component
{
        ...
        if (strpos($propertyName, 'products.') === 0 && strpos($propertyName, 'quantity') !== false) {
            ...
            if (!empty($productId)) {
                ...
                if ($product) {
                    ...
                    $this->products[$key]['subtotal'] = $quantity * $this->products[$key]['unit_price'];
                }
            }
        }
    }
    ...
    public function save()
    {
        ...
        try {
            ...
            // Reset products array and flash success message
            $this->products = [];
            session()->flash('message', 'Transaction Details Successfully Updated.');

            // Optional: refresh the page after save
    ...
    public function removeProductDetail($detailId)
    {
        ...
        if ($detail) {
            ...
            session()->flash('message', 'Product removed from transaction.');
        } else {
    ...
    public function render()
    {
        $transactionDetails = TransactionDetail::where('transaction_id', $this->transactionId)
            ->with('product') // Pastikan relasi ke produk ada
            ->get();
